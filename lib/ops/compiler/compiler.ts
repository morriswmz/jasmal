...
import { OutputDTypeCalculator, DType, DTypeHelper } from '../../dtype';
...
export interface UnaryEWOpConfig {
    outputDTypeCalculator?: (t: DType, isComplex: boolean) => DType | undefined;
    noInPlaceOperation?: boolean;
}

export interface BinaryEWOpConfig {
    outputDTypeCalculator?: (t1: DType, isComplex1: boolean, t2: DType, isComplex2: boolean) => DType | undefined;
    noInPlaceOperation?: boolean;
}
...
export class TensorElementWiseOpCompiler {
    ...
    /**
    * Checks if the broadcasting is possible between the two tensors.
    ...
    */
    ...
    private _getUnaryOpDependencies(opConfig?: UnaryEWOpConfig): UnaryOpDependencies {
        return {
            ...
            determineOutputType: opConfig && opConfig.outputDTypeCalculator
                ? opConfig.outputDTypeCalculator : OutputDTypeCalculator.uNoChange,
    ...
    public makeBinaryOp(opTemplate: BinaryEWOpTemplate,
        ...
        let outputDTypeCalculator = (config && config.outputDTypeCalculator)
            ? config.outputDTypeCalculator
            : OutputDTypeCalculator.bWider;
        let deps: BinaryOpDependencies = {
            Tensor: Tensor,
            ComplexNumber: ComplexNumber,
            computeStrides: ShapeHelper.computeStrides,
            compareShape: ShapeHelper.compareShape,
            determineOutputType: outputDTypeCalculator,
