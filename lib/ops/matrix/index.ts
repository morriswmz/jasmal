...
import { NormFunctions } from './norm';
...
export class MatrixOpProviderFactory {
    ...
    public static create(arithmOp: IArithmeticOpProvider, mbs: IMatrixBasicSubroutines = new BuiltInMBS()): IMatrixOpProvider {
        ...
        const opHilb = (n: number, dtype: DType = DType.FLOAT64) => {
            if (dtype !== DType.FLOAT64 && dtype !== DType.FLOAT32) {
                throw new Error('Data type must be float.');
            }
            let m = Tensor.zeros([n, n], dtype);
            for (let i = 0;i < n;i++) {
                for (let j = 0;j < n;j++) {
        ...
        const matMulOutputTypeCalculator = (t1: DType, isComplex1: boolean, t2: DType, isComplex2: boolean): DType | undefined => {
        ...
        const opMatMul = (x: OpInput, y: OpInput, yModifier: MatrixModifier = MatrixModifier.None): OpOutput => {
            ...
            let Z = Tensor.zeros([m, p],
                matMulOutputTypeCalculator(vx.originalDType, vx.isComplex, vy.originalDType, vy.isComplex));
            if (vx.isComplex) {
            ...
            if (X.ndim === 1 || (X.ndim === 2 && X.shape[1] === 1)) {
                ...
                switch (p) {
                    case 0:
                        return X.hasComplexStorage()
                            ? NormFunctions.cvec0Norm(X.realData, X.imagData)
                            : NormFunctions.vec0Norm(X.realData);
                    case 2:
                        return X.hasComplexStorage()
                            ? NormFunctions.cvec2Norm(X.realData, X.imagData)
                            : NormFunctions.vec2Norm(X.realData);
                    case Infinity:
                        return X.hasComplexStorage()
                            ? NormFunctions.cvecInfNorm(X.realData, X.imagData)
                            : NormFunctions.vecInfNorm(X.realData);
                    default:
                        return X.hasComplexStorage()
                            ? NormFunctions.cvecPNorm(X.realData, X.imagData, p)
                            : NormFunctions.vecPNorm(X.realData, p);
                }
            } else {
                // matrix norm
                let shape = X.shape;
                if (typeof(p) === 'string') {
                    ...
                    switch (p) {
                        case 1:
                            return X.hasComplexStorage()
                                ? NormFunctions.cmat1Norm(shape[0], shape[1], X.realData, X.imagData)
                                : NormFunctions.mat1Norm(shape[0], shape[1], X.realData);
                        case 2:
                            return X.hasComplexStorage()
                                ? NormFunctions.cmat2Norm(shape[0], shape[1], X.realData, X.imagData)
                                : NormFunctions.mat2Norm(shape[0], shape[1], X.realData);
                        case Infinity:
                            return X.hasComplexStorage()
                                ? NormFunctions.cmatInfNorm(shape[0], shape[1], X.realData, X.imagData)
                                : NormFunctions.matInfNorm(shape[0], shape[1], X.realData);
                        default:
