    ...
    public static csvd(m: number, n: number, computeUV: boolean, reA: DataBlock,
        ...
        for (i = 0;i < n;i++) {
            ...
            if (i < m) {
                ...
                // transform matrix.
                for (k = i;k < m;k++) {
                    scale += CMath.length2(reA[k * n + i], imA[k * n + i]);
                }
                if (scale) {
                    // Scale is non-zero. If scale is zero, we do not need to do
                    // anything.
                    ...
                    // now s = ||A[i,i:end]||_2^2 / scale^2
                    // f <- |A[i,i]|
                    f = CMath.length2(reA[i * n + i], imA[i * n + i]);
            ...
            if (i < m && i !== n - 1) {
                for (k = l;k < n;k++) {
                    scale += CMath.length2(reA[i * n + k], imA[i * n + k]);
                }
                if (scale) {
                    for (k = l;k < n;k++) {
                        reA[i * n + k] /= scale;
                    ...
                    f = CMath.length2(reA[i * n + l], imA[i * n + l]);
                    g = Math.sqrt(s);
                    h = - f * g - s;
                    if (f) {
        ...
        if (computeUV) {
            ...
            for (i = n - 1;i >= 0;i--) {
                if (i < n - 1) {
                    if (g) {
                        ...
                        h = - 1.0 / CMath.length2(reA[i * n + l], imA[i * n + l]) / g;
                        for (j = l;j < n;j++) {
                            accRe = 0;
                            accIm = 0;
                            // w_i^H Q[i+1:end,j]
            ...
            for (i = Math.min(m, n) - 1;i >= 0;i--) {
                ...
                if (g) {
                    // update U[i:end,i+1:end]
                    // h is now -\beta
                    h = - 1.0 / CMath.length2(reA[i * n + i], imA[i * n + i]) / g;
                    for (j = l;j < n;j++) {
                        // acc <- w_2^H * D
                        accRe = 0;
                        accIm = 0;
