...
describe('fft()/ifft()', () => {
    ...
    it('should compute the FFT of a real vector whose length is not a power of two', () => {
        let actual = T.fft([1, 2, 3, 2, 1]);
        let expected = T.fromArray([
            9,
            -2.1180339887498949,
            1.1803398874989490e-1,
            1.1803398874989490e-1,
            -2.1180339887498949
        ], [
            0,
            -1.5388417685876266,
            3.6327126400268039e-1,
            -3.6327126400268039e-1,
            1.5388417685876266
        ]);
    ...
    it('ifft(fft(X, 1), 1) should give back X for a random tensor X', () => {
        let shape = [3, 16, 7];
    ...
    it('ifft(fft(X, 0), 0) should give back X for a complex tensor X generated by a phaser', () => {
        let phase = T.transpose(T.linspace(0, Math.PI * 5, 4000));
        let init = T.randn([1, 2]);
        let X = <Tensor>T.exp(T.mul(T.add(phase, init), T.J, true));
        let XCopy = X.copy(true);
        let actual = T.ifft(T.fft(X, 0), 0);
        // not very accurate for long sequences?
        checkTensor(actual, X, 1e-7);
        // should not change X
        checkTensor(X, XCopy);
    });
});
